...
namespace Windows
{
    ...
    ActiveProcessesSupervisor::extractProcessInformation(uint64_t eprocessBase) const
    {
        ...
        const auto vadTree = std::make_unique<VadTreeWin10>(
            kernelAccess, eprocessBase, processInformation->pid, processInformation->name, loggingLib);
        const auto vadtList = vadTree->getAllVadts();
        processInformation->memoryRegions = std::make_unique<std::vector<MemoryRegion>>();

        for (const auto& vadtListElement : *vadtList)
        {
            const auto startAddress = vadtListElement.startingVPN << PagingDefinitions::numberOfPageIndexBits;
            const auto endAddress = ((vadtListElement.endingVPN + 1) << PagingDefinitions::numberOfPageIndexBits) - 1;
            ...
            processInformation->memoryRegions->emplace_back(
                startAddress,
                size,
                vadtListElement.fileName,
                ::PageProtection(static_cast<uint32_t>(vadtListElement.protection), OperatingSystem::WINDOWS),
    ...
    ActiveProcessesSupervisor::getProcessInformationByBase(uint64_t eprocessBase)
    {
