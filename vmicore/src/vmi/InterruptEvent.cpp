...
{
    targetPAString = Convenience::intToHex(targetPA);
...
void InterruptEvent::setupVmiInterruptEvent()
{
    ...
    {
        throw VmiException(std::string(__func__) + "Interrupt already registered at this address: " + targetPAString);
    }
    ...
    try
    {
        ...
        else
        {
            GlobalControl::logger()->debug(
                ...
                {logfield::create("logger", loggerName), logfield::create("eventPA", Convenience::intToHex(eventPA))});
...
event_response_t InterruptEvent::interruptCallback(uint32_t vcpuId)
{
    ...
    catch (const std::runtime_error& e)
    {
        throw std::runtime_error(std::string(__func__) + ": " + e.what() +
                                 " Target physical address = " + targetPAString);
    }
    disableEvent();
