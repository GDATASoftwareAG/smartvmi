    ...
    class MockPluginInterface : public PluginInterface
    {
      public:
        MOCK_METHOD(std::unique_ptr<std::vector<uint8_t>>,
                    ...
                    (const, override));
        MOCK_METHOD(std::unique_ptr<std::vector<MemoryRegion>>,
                    getProcessMemoryRegions,
                    (pid_t pid),
                    (const, override));
        MOCK_METHOD(std::unique_ptr<std::vector<ProcessInformation>>, getRunningProcesses, (), (const, override));
        MOCK_METHOD(std::unique_ptr<std::vector<std::string>>, getLoadedModules, (pid_t processID), (const, override));
        MOCK_METHOD(void,
                    registerProcessTerminationEvent,
                    (processTerminationCallback_f terminationCallback),
                    (override));
        MOCK_METHOD(void, registerProcessStartEvent, (processStartCallback_f startCallback), (override));
