...
void Scanner::scanProcess(pid_t pid, const std::string& processName)
{
    ...
    {
        pluginInterface->logMessage(
            LogLevel::info, LOG_FILENAME, "Scanning process " + std::to_string(pid) + " \"" + processName + "\"");
        try
        {
        ...
        pluginInterface->logMessage(
            LogLevel::info, LOG_FILENAME, "Done scanning process " + std::to_string(pid) + " \"" + processName + "\"");
    }
}
